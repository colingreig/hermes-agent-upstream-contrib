...
# Keep the import-time identity so tests/integrators that replace the stdlib
# executor can still inject their implementation. Normal production runs use
# the daemonized subclass below so a stuck cron worker cannot block shutdown.
_STDLIB_THREAD_POOL_EXECUTOR = concurrent.futures.ThreadPoolExecutor
...
def _run_job_script(
    ...
    * anything else — run with the current Python interpreter
    ...
    try:
        ...
        subprocess_env.update(env_overlay)
...
def run_job(
    ...
    try:
        ...
        if _session_db_timeout > 0:
            _session_db_pool = _DaemonThreadPoolExecutor(max_workers=1)
            try:
    ...
    try:
        ...
        _cron_executor_cls = (
            ...
            if concurrent.futures.ThreadPoolExecutor is _STDLIB_THREAD_POOL_EXECUTOR
            else concurrent.futures.ThreadPoolExecutor
        )
        _cron_pool = _cron_executor_cls(max_workers=1)
        ...
        if _session_db:
            ...
            try:
                ...
                if not _set_cron_session_title(
                    _session_db, _resolved_cron_session_id, _cron_title
                ):
                    # Helper returned None (blank base) -> use the id fallback.
                    _set_cron_session_title(
                        _session_db, _resolved_cron_session_id, f"cron {job_id}"
                    )
            except (Exception, KeyboardInterrupt) as e:
                logger.debug(
                ...
                for _fallback in (
                    ...
                    f"cron {job_id} {_resolved_cron_session_id[-6:]}",
                ):
                    try:
                        if _set_cron_session_title(
                            _session_db, _resolved_cron_session_id, _fallback
                        ):
                            break
