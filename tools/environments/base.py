...
class BaseEnvironment(ABC):
    ...
    def init_session(self):
        ...
        #
        # The temp name MUST be unique per concurrent writer. ``BASHPID`` is
        # the actual subshell PID on modern Bash; macOS still ships Bash 3.2,
        # where that variable is unset, so fall back to ``$$``. Each Hermes
        # command runs in its own Bash process, making ``$$`` unique across the
        # concurrent writers on that platform too. The static path is quoted
        # while the numeric expansion remains outside the quotes.
        _writer_pid = "${BASHPID:-$$}"
        _snap_tmp = self._quote_shell_path(self._snapshot_path + ".tmp.") + _writer_pid
        bootstrap = (
            f"umask 077\n"
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        # truncated/half-written file. Prefer the per-subshell ``BASHPID`` on
        # modern Bash and fall back to the process PID on macOS Bash 3.2. Every
        # command is launched in a distinct Bash process, so either value is
        # unique across concurrent writers. Static path shell-quoted; the
        # numeric expansion stays outside the quotes.
        _writer_pid = "${BASHPID:-$$}"
        _snap_tmp = self._quote_shell_path(self._snapshot_path + ".tmp.") + _writer_pid
