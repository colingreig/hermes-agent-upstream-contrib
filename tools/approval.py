...
def _is_verification_artifact_cleanup(command: str) -> bool:
    ...
    operand = argv[2]
    configured_temp_dir = os.path.normpath(tempfile.gettempdir())
    temp_dir = os.path.realpath(configured_temp_dir)
    basename = os.path.basename(operand)
    accepted_operand = os.path.join(temp_dir, basename)
    # macOS exposes /tmp as the fixed system alias /private/tmp. TMPDIR=/tmp
    # therefore produces legitimate verification artifacts using the alias
    # spelling even though the canonical temp directory is /private/tmp. Keep
    # this exception exact: arbitrary symlinked temp dirs remain rejected by
    # the canonical-target test below.
    is_macos_tmp_alias = configured_temp_dir == "/tmp" and temp_dir == "/private/tmp"
    if operand != accepted_operand and not (
        is_macos_tmp_alias and operand == os.path.join(configured_temp_dir, basename)
    ):
        return False
