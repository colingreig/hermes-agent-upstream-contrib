...
def test_profile_global_fallback_normalizes_in_memory_without_writing(tmp_path, monkeypatch):
    ...
    for key in ("ANTHROPIC_API_KEY", "ANTHROPIC_TOKEN", "CLAUDE_CODE_OAUTH_TOKEN"):
        monkeypatch.delenv(key, raising=False)
    # This assertion is specifically about normalizing a row borrowed from the
    # global fallback store. Upstream pool loading also discovers live Hermes /
    # Claude Code OAuth credentials and correctly persists those newly seeded
    # references into the active profile, so isolate that independent write
    # path rather than depending on the developer machine's auth state.
    monkeypatch.setattr(
        "agent.anthropic_adapter.read_claude_code_credentials",
        lambda: None,
    )
