...
class TestFreshFinalForLongLivedPreviews:
    ...
    async def test_long_lived_preview_sends_fresh_final(self):
        ...
        await consumer._send_or_edit("hello")
        # Force the preview to look stale (visible for > 60s). Offset from
        # the current monotonic reading rather than pinning to 0.0 — on
        # hosts/containers where CLOCK_MONOTONIC's epoch is recent (fresh
        # CI runner, low uptime), an absolute 0.0 can be *less* than 60s
        # in the past, silently defeating the "stale" simulation.
    ...
    async def test_only_finalize_triggers_fresh_final(self):
        ...
        await consumer._send_or_edit("hello")
        consumer._message_created_ts = time.monotonic() - 61.0  # stale
...
class TestCancelledBestEffortDeliveryFinalizes:
    ...
    async def test_cancel_with_fresh_final_enabled_delivers_and_flags_via_handler(self):
        ...
        task = asyncio.create_task(consumer.run())
        await asyncio.sleep(0.05)
        consumer._message_created_ts = time.monotonic() - 1.0  # force the preview stale
        task.cancel()
        await asyncio.gather(task, return_exceptions=True)
