...
# ---------------------------------------------------------------------------
# TestDecisionThreadHookAuthorization
# ---------------------------------------------------------------------------


class TestDecisionThreadHookAuthorization:
    @staticmethod
    def _make_event():
        return {
            "text": "Use option A",
    ...
    async def test_unauthorized_or_unknown_sender_cannot_trigger_hook(
        self, adapter, authorization
    ):
        with patch.object(
            adapter, "_is_sender_authorized", return_value=authorization
            ...
            adapter, "_resolve_user_name", new=AsyncMock(return_value="Operator")
        ), patch.object(
            _slack_mod, "_match_decision_thread", return_value="86task"
        ) as match_thread, patch.object(
            _slack_mod, "_post_clickup_operator_comment", new=AsyncMock()
        ) as post_comment:
            await adapter._handle_slack_message(self._make_event())

        authorize.assert_called_once_with(
            "U_OPERATOR", chat_type="thread", chat_id="D123"
        )
        match_thread.assert_not_called()
        post_comment.assert_not_awaited()
        adapter.handle_message.assert_awaited_once()

    @pytest.mark.asyncio
    async def test_authorized_sender_can_trigger_hook(self, adapter):
        adapter.send = AsyncMock()
        with patch.object(
            adapter, "_is_sender_authorized", return_value=True
        ) as authorize, patch.object(
            _slack_mod, "_match_decision_thread", return_value="86task"
        ) as match_thread, patch.object(
            _slack_mod,
            "_post_clickup_operator_comment",
        ...
        match_thread.assert_called_once_with("D123", "2000.0")
        post_comment.assert_awaited_once_with("86task", "Use option A")
