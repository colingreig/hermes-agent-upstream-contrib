...
def _real_binary_contract_available(tool: str, *, needs_tty: bool = False) -> bool:
    """Return whether this host provides the GNU CLI grammar pinned below.

    The security detector remains covered on every platform by the pure unit
    tests in this file. These subprocess cases are supplementary executable
    documentation for GNU sort/man-db and util-linux script semantics.
    """
    ...
    try:
        if tool == "sort":
            version = subprocess.run(
                [tool, "--version"], text=True, capture_output=True, timeout=2
            )
            ...
            if "man-db" not in version.stdout:
                return False
        if needs_tty:
            script = subprocess.run(
                ["script", "--version"], text=True, capture_output=True, timeout=2
            )
            if "util-linux" not in (script.stdout + script.stderr):
                return False
...
def test_real_read_tool_binaries_confirm_option_ownership(
    ...
    if not _real_binary_contract_available(argv[0]):
        pytest.skip(f"GNU {argv[0]} contract is unavailable")
...
def test_real_binaries_execute_leading_dash_program_payload(
    ...
    if not _real_binary_contract_available(tool, needs_tty=needs_tty):
        pytest.skip(f"GNU {tool}/util-linux script contract is unavailable")
