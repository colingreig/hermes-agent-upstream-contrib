...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Lifecycle events keep Slack's Agent/Home surface synchronized but
            # do not enter the agent loop, so they remain active in notify-only
            # mode while text/file/slash ingress above stays disabled.
    ...
    async def _handle_slack_message(
        ...
        if is_thread_reply:
            try:
                ...
                if not _is_own and self._is_sender_authorized(
