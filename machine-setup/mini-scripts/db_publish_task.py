...
WHAT IT DOES (atomic, no LLM glue):
  ...
  4. opencode_exec.py --workdir ~/.hermes/worktrees/ignite-<taskId> --prompt-file <p> --task-id <id>
     (NO --model — writer cascade). Require ok:true AND mode=="db-publish" AND a non-empty
...
def main():
    ...
    # 4. OpenCode (CONTENT cascade: Sonnet > GLM-5.2 > Gemini; NO --model) -------
    # --content selects the prose cascade — blog rewrites are file-based content.
