...
def _get_packaged_data_dir(name: str) -> Optional[Path]:
...
def get_optional_mcps_dir(default: Optional[Path] = None) -> Path:
...
def get_bundled_skills_dir(default: Optional[Path] = None) -> Path:
...
def iter_hermes_node_dirs(home: Optional[Path] = None) -> list:
...
def node_tool_runnable(path: Optional[str]) -> bool:
...
def hermes_managed_node_tree_present(home: Optional[Path] = None) -> bool:
...
def find_hermes_node_executable(command: str) -> Optional[str]:
...
def find_node_executable_on_path(command: str) -> Optional[str]:
...
def agent_browser_runnable(path: Optional[str]) -> bool:
...
def _norm_home_path(path: Optional[str]) -> str:
...
def _profile_home_path(env: Optional[dict] = None) -> Optional[str]:
...
def _is_profile_home(candidate: Optional[str], profile_home: Optional[str]) -> bool:
    return bool(candidate and profile_home and _norm_home_path(candidate) == _norm_home_path(profile_home))


def _iter_real_home_candidates(env: Optional[dict] = None) -> list:
...
def get_real_home(env: Optional[dict] = None) -> str:
...
def get_subprocess_home(env: Optional[dict] = None) -> Optional[str]:
...
def parse_reasoning_effort(effort) -> Optional[dict]:
...
def resolve_per_model_reasoning_effort(model: str, overrides: Optional[dict]) -> Optional[dict]:
...
def resolve_reasoning_config(cfg: Optional[dict], model: str = "") -> Optional[dict]:
...
def windows_path_to_wsl(path: str) -> Optional[str]:
...
def wsl_unc_path_to_posix(path: str) -> Optional[str]:
