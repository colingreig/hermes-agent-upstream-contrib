#!/usr/bin/env python3
"""Manifest-verified installer for the hermes-github-app deploy bundle.

This script is the SOLE writer of the three GitHub App auth artifacts
(``github_app_token.py``, ``github_app_cred.sh``, ``thermal_github_app_probe.py``)
into ``~/.hermes/scripts/``. It reads ``github_app_manifest.json``, verifies
every source's sha256 against the manifest, snapshots each existing destination,
then atomically installs and re-verifies the deployed bytes.

``github_app_token.py`` is also refreshed by ``reconcile_launchd_environment.py``
on release cut; ``github_app_cred.sh`` and ``thermal_github_app_probe.py``
previously had no manifest and no governed deploy path — the same gap that let
the 86e2hap1g spend fix ship as v0.18.2 without reaching the live mini copy.
...
Usage:
    install_github_app.py --dry-run
    install_github_app.py
"""
...
def _print_plan(plan: list[dict], warnings: list[str], home: Path) -> None:
    print(f"hermes-github-app install plan (home={home}):")
    for item in plan:
        print(
...
def install(
    ...
    *,
    ...
    stamp = _utc_stamp()
    snapshot_dir = home / ".hermes" / "logs" / "github-app-installs" / stamp
    ...
    try:
        for item in plan:
            ...
            if pre_existed:
                ...
                shutil.copy2(dest, dest.with_name(dest.name + f".bak-github-app-install-{stamp}"))
    ...
    receipt = {
        "bundle": manifest.get("bundle", "hermes-github-app"),
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--manifest",
        type=Path,
        default=here / "github_app_manifest.json",
        help="path to github_app_manifest.json (default: alongside this script)",
    )
    parser.add_argument(
        "--home",
        type=Path,
