...
Before this installer existed, the four spend-control files had no manifest
and no governed deploy path at all: a release could change
``spend_guard.py`` or ``spend_opencode.py`` in the repo and mini-release-cut.sh
would still print "governed script deployment verified," because that check
only ever covered ``clickup_workspace_refresh.py``. The fix landed in
``spend_opencode.py``/``spend_guard.py``/``spend_meter.py`` for the
subscription-aware billing-route bug (86e2hap1g) was merged and released as
v0.18.2 but never reached the live ``~/.hermes/scripts/`` copy the mini
cron actually executes — see ``hermes-cron-executes-from-scripts-dir-not-release``.
This bundle+installer closes that gap the same way ``self_report_manifest.json``
...
BLOCKED_DEST_BASENAMES = {"claim_store.py", "hermes_report_build.py"}
...
def _print_plan(plan: list[dict], warnings: list[str], home: Path) -> None:
    print(f"hermes-spend-guard install plan (home={home}):")
    for item in plan:
        print(
...
def install(
    ...
    *,
    ...
    stamp = _utc_stamp()
    snapshot_dir = home / ".hermes" / "logs" / "spend-guard-installs" / stamp
    ...
    receipt = {
        "bundle": manifest.get("bundle", "hermes-spend-guard"),
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--manifest",
        type=Path,
        default=here / "spend_manifest.json",
        help="path to spend_manifest.json (default: alongside this script)",
    )
    parser.add_argument(
        "--home",
        type=Path,
