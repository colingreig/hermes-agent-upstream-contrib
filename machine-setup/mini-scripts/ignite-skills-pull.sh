...
EVIDENCE="$HERMES_HOME/state/skill-pulls/ignite-skills-live-success.json"
RECORDER="$HERMES_HOME/scripts/record_skill_pull_success.py"
GUARD="$HERMES_HOME/scripts/skill_pull_guard.py"
LOG="$HERMES_HOME/logs/ignite-skills-pull.log"
LOCK="$HERMES_HOME/state/skill-pulls/ignite-skills-live.lock"
...
cleanup() {
  ...
  if [ "$UPDATING_PUBLISHED" -eq 1 ] && [ "$PULL_SUCCEEDED" -eq 0 ]; then
    "$PYTHON" "$RECORDER" --source ignite-skills-live \
...
trap cleanup EXIT
"$PYTHON" "$GUARD" clean --root "$ROOT" --path skills --path ignite-content/skills
...
"$PYTHON" "$RECORDER" --target "$EVIDENCE" --source ignite-skills-live \
  --root "$ROOT" --commit "$COMMIT" --generation-target "$GENERATION" \
  --changed-from "$BEFORE" --operation-id "$CATALOG_LOCK_TOKEN"
PULL_SUCCEEDED=1
UPDATING_PUBLISHED=0
printf '%s pull OK source=ignite-skills-live commit=%s\n' \
