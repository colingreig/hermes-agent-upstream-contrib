...
5. **Install bundle**: `install_fleet_config.py --dry-run` first, then real run — applies overlay (model/fallbacks/kanban), installs 5 profiles to `~/.hermes/profiles/`, replaces `cron/jobs.json` (15 jobs), and applies the manifest-pinned skill policy across the default plus five profile homes. Skill removals are archived, the two poller references are hash-verified into the kept umbrella archive, and the local vehicle-image shadow requires exact hub-lock provenance. Timestamped .baks, skill tarballs, and the receipt are automatic.
