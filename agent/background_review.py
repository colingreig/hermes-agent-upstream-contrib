...
_SKILL_REVIEW_PROMPT = (
    "Review the conversation above and actively update the skill library ONLY "
    "when a genuinely novel, transferable, cross-session lesson emerged. "
    "Most routine cron ticks produce NOTHING worth saving — 'Nothing to "
    "save.' is the correct and common outcome. Do not manufacture an "
    "update to feel productive; skipping a real learning opportunity is a missed "
    "opportunity for the skill library.\n\n"
    "⚠️ SIZE DISCIPLINE (hard rule): a SKILL.md body has a 100,000-char "
    "HARD LIMIT — saves past it are REJECTED and waste the whole pass. "
    "NEVER append to a SKILL.md body that is already large (>50KB): put "
    "any genuinely new detail in `references/<topic>.md` and add at most "
    "a one-line pointer. NEVER write dated per-tick status, confirmation, "
    "or 'tick NN observed' notes into SKILL.md OR into references/ — that "
    "is operational state and belongs in memory or the brain, never the "
    "skill library. If your only candidate is such a note, say 'Nothing "
    "to save.' and stop.\n\n"
    ...
    "'Nothing to save.' is the right default for routine ticks. If the "
    "session ran smoothly with no corrections and produced no new "
    "transferable technique, say 'Nothing to save.' and stop. Only act "
    "when a real, durable, cross-session lesson emerged."
)
...
_COMBINED_REVIEW_PROMPT = (
    ...
    "**Skills**: how to do this class of task. Stay ACTIVE and update ONLY on a "
    "genuinely novel, transferable lesson; most routine ticks save "
    "nothing and 'Nothing to save.' is correct. Staying active here means not "
    "treating a likely learning opportunity as a neutral no-op. ⚠️ SIZE DISCIPLINE: "
    "SKILL.md bodies have a 100,000-char hard limit (saves rejected past "
    "it). Never append to a SKILL.md already >50KB — use `references/` + "
    "a one-line pointer. Never write dated per-tick status/confirmation "
    "notes to the skill library; those go to memory or the brain.\n\n"
